...
function FoldNumbers:new() return setmetatable({}, self) end
...
function FoldNumbers:apply(ast)
  ...
  visitast(ast, nil, function(node)
    ...
    or node.kind == K.ModExpression or node.kind == K.PowExpression then
      ...
      and type(lhs.value) == 'number' and type(rhs.value) == 'number' then
  ...
  if changed > 0 then print('[FoldNumbers] dobrou ' .. changed .. ' expressões numéricas') end
  return ast
end
