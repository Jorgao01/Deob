...
function UndoProxifyLocals:new() return setmetatable({}, self) end
...
local function find_value_name_keys(ast)
  ...
  visitast(ast, nil, function(n)
    if n.kind == K.AssignmentStatement then
      ...
      if lhs and lhs.kind == K.AssignmentIndexing
      and rhs and (rhs.kind == K.VariableExpression
               or rhs.kind == K.FunctionLiteralExpression) then
...
function UndoProxifyLocals:apply(ast)
  ...
  visitast(ast, nil, function(node)
    if node.kind == K.VariableExpression then
      local vn = valueNames[node.scope] and valueNames[node.scope][node.id]
      if vn then return Ast.VariableExpression(node.scope, node.id) end
    end
    if node.kind == K.AssignmentVariable then
      local vn = valueNames[node.scope] and valueNames[node.scope][node.id]
      if vn then return Ast.AssignmentVariable(node.scope, node.id) end
    end
  end)
  return ast
end
