...
local function key_of(var) return tostring(var.scope) .. ':' .. tostring(var.id) end

function EnvNormalize:new() return setmetatable({}, self) end
...
local function collect_aliases(ast)
  ...
  visitast(ast, nil, function(node)
    ...
    and node.expressions and #node.ids == 1 and #node.expressions == 1 then
      local expr = node.expressions[1]
      if expr and expr.kind == Ast.AstKind.IndexExpression then
        local base, idx = expr.base, expr.index
        if base and base.kind == Ast.AstKind.VariableExpression
        and idx and idx.kind == Ast.AstKind.StringExpression then
          if whitelist[idx.value] then
            aliases[key_of({ scope=node.scope, id=node.ids[1] })] = idx.value
          end
        end
      elseif expr and expr.kind == Ast.AstKind.VariableExpression then
        ...
        if whitelist[vname] then
          aliases[key_of({ scope=node.scope, id=node.ids[1] })] = vname
        end
      end
    end
  end)
...
local function transform(ast, aliases)
  visitast(ast, nil, function(node)
    if node.kind == Ast.AstKind.VariableExpression then
      local name = aliases[key_of(node)]
    ...
    elseif node.kind == Ast.AstKind.IndexExpression then
      ...
      if base and base.kind == Ast.AstKind.VariableExpression
      and idx and idx.kind == Ast.AstKind.StringExpression
      and whitelist[idx.value] then
        return make_global(ast, idx.value)
      end
    end
  end)
end
  ...
  for _, st in ipairs(block.statements or {}) do
    if st.kind == Ast.AstKind.LocalVariableDeclaration then
      ...
      for _, id in ipairs(st.ids) do
        if aliases[key_of({ scope=st.scope, id=id })] then drop=true; break end
      end
