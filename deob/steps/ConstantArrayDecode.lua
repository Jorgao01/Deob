-- ConstantArrayDecode.lua (sem alterações de lógica, apenas cleanup de estilo)
...
local function find_block(src, start_pat, open_ch, close_ch)
  ...
  while i <= #src do
    ...
    if start_i then
      ...
      if ch == close_ch then
        depth = depth - 1
        if depth == 0 then return start_i, i - 1 end
      end
    else
...
local function parse_rotate_pairs(src)
  ...
  local function pair_to_nums(s)
    ...
    if not a then
      s = s:sub(2, -2)
      local pp = {}
      for x in s:gmatch('[^,};]+') do table.insert(pp, x) end
      a, b = pp[1], pp[2]
    end
...
local function decode_base64_custom(s, map)
  ...
  while idx <= len do
    ...
    if code then
      ...
      count = count + 1
      if count == 4 then count = 0; push3(value); value = 0 end
    elseif ch == '=' then
...
local function eval_num_expr(node)
  ...
  if node.kind == K.AddExpression or node.kind == K.SubExpression
  or node.kind == K.MulExpression or node.kind == K.DivExpression
  or node.kind == K.ModExpression or node.kind == K.PowExpression then
    ...
    if node.kind == K.DivExpression and b ~= 0 then return a / b end
    if node.kind == K.ModExpression and b ~= 0 then return a % b end
...
local function replace_value_node(val)
  ...
  if type(val) == 'boolean' then return Ast.BooleanExpression(val) end
...
function ConstantArrayDecode:apply(ast, pipeline)
  ...
  -- Pass 1: coleta referências
  visitast(ast, nil, function(node)
    if node.kind == Ast.AstKind.LocalVariableDeclaration
    and node.expressions and #node.expressions == 1 then
      local expr = node.expressions[1]
      if expr.kind == Ast.AstKind.TableConstructorExpression and not arrScope then
      ...
      if body and #body.statements == 1 then
        ...
        if st.kind == Ast.AstKind.ReturnStatement and #st.args == 1 then
          ...
          if ret.kind == Ast.AstKind.IndexExpression then
            ...
            if (index.kind == Ast.AstKind.AddExpression
            or  index.kind == Ast.AstKind.SubExpression)
            and base.kind == Ast.AstKind.VariableExpression then
              local lhs, rhs = index.lhs, index.rhs
              if lhs.kind == Ast.AstKind.VariableExpression
              and rhs.kind == Ast.AstKind.NumberExpression then
                wrapperScope = node.scope; wrapperId = node.id
                wrapperName  = node.scope.getVariableName
                  and node.scope:getVariableName(node.id) or nil
                arrScope = base.scope; arrId = base.id
                wrapperOffset = (index.kind == Ast.AstKind.AddExpression)
                  and rhs.value or -rhs.value
              end
            end
          end
        end
    ...
    -- coleta wrappers locais em tabela
    if node.kind == Ast.AstKind.LocalVariableDeclaration
    and #node.ids == 1
    and node.expressions and node.expressions[1]
    and node.expressions[1].kind == Ast.AstKind.TableConstructorExpression then
      local tbl = node.expressions[1]
      for _, entry in ipairs(tbl.entries or {}) do
        if entry.kind == Ast.AstKind.KeyedTableEntry
        and entry.key.kind == Ast.AstKind.StringExpression then
          local key = entry.key.value
          if entry.value.kind == Ast.AstKind.FunctionLiteralExpression then
            local fn    = entry.value
            local fbody = fn.body
            if fbody and #fbody.statements == 1
            and fbody.statements[1].kind == Ast.AstKind.ReturnStatement then
              local call = fbody.statements[1].args[1]
              if call and call.kind == Ast.AstKind.FunctionCallExpression then
                local idxExpr
                if call.base.kind == Ast.AstKind.IndexExpression then
                  idxExpr = call.base.index
                end
                if idxExpr and (idxExpr.kind == Ast.AstKind.AddExpression
                             or idxExpr.kind == Ast.AstKind.SubExpression) then
                  local lhs, rhs = idxExpr.lhs, idxExpr.rhs
                  if lhs.kind == Ast.AstKind.VariableExpression
                  and rhs.kind == Ast.AstKind.NumberExpression then
                    local off = (idxExpr.kind == Ast.AstKind.AddExpression)
                      and rhs.value or -rhs.value
                    localWrappers[key] = {
                      scope = node.scope, id = node.ids[1],
                      offset = off, fn = fn,
                    }
                  end
                end
  ...
  -- Pass 2: substitui indexações diretas no array
  visitast(ast, nil, function(node)
    if node.kind == Ast.AstKind.IndexExpression
    and node.base.kind == Ast.AstKind.VariableExpression
    and arrScope and arrId
    and node.base.scope == arrScope
    and node.base.id == arrId then
      if node.index.kind == Ast.AstKind.NumberExpression
      and decoded[node.index.value] ~= nil then
        replaced = replaced + 1
        return replace_value_node(decoded[node.index.value])
      end
    end
  end)

  -- Pass 3: substitui chamadas ao wrapper
  if arrScope and arrId and wrapperOffset ~= nil then
    visitast(ast, nil, function(node)
      if node.kind == Ast.AstKind.FunctionCallExpression
      and node.base.kind == Ast.AstKind.VariableExpression then
        local ok = false
        if wrapperScope and wrapperId
        and node.base.scope == wrapperScope
        and node.base.id == wrapperId then ok = true end
        if not ok and wrapperName and node.base.scope
        and node.base.scope.getVariableName
        and node.base.scope:getVariableName(node.base.id) == wrapperName then
          ok = true
        end
        if ok and #node.args == 1 then
          ...
          if num then
            ...
            if decoded[idx] ~= nil then
              replaced = replaced + 1
              return replace_value_node(decoded[idx])
            end
          end
        end
      end
  ...
  -- Pass 4: substitui wrappers em tabela local
  if arrScope and arrId and next(localWrappers) then
    visitast(ast, nil, function(node)
      if node.kind == Ast.AstKind.FunctionCallExpression
      and node.base.kind == Ast.AstKind.IndexExpression then
        ...
        if base.base.kind == Ast.AstKind.VariableExpression then
          ...
          if base.index.kind == Ast.AstKind.StringExpression then
            info = localWrappers[base.index.value]
          end
          if info
          and base.base.scope == info.scope
          and base.base.id == info.id then
            local num
  ...
  if replaced > 0 then
    print('[ConstantArrayDecode] inlinizou ' .. replaced .. ' constantes')
  end
  return ast
end
