...
function UndoSplitStrings:new() return setmetatable({}, self) end
...
local function custom1_join(argtb)
  ...
  for i = 1, n do
    ...
    local strEntry = strtb.entries[idx]
    if not strEntry then return nil end
    parts[#parts + 1] = strEntry.value.value
  end
...
function UndoSplitStrings:apply(ast)
  ...
  if count > 0 then print('[UndoSplitStrings] reconstruiu ' .. count .. ' strings') end
  return ast
end
